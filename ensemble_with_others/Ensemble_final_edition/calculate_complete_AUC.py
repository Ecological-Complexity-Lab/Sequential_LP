...
def creat_numpy_files_predictor_choice(dir_results, name, predict, choice):
    ...
    if choice == 0 :
        ...
        full_feat_set = ["i", "j", "lstm1", "lstm2", "lstm3", "lstm4"]
    
    elif choice ==4:
        full_feat_set = base_top_set + ts_col + ["sbm"] +  ["lstm1", "lstm2", "lstm3", "lstm4"]
    ...
    X_train_orig = X_train_orig.replace([np.inf, -np.inf], 0)
    X_test_orig = X_test_orig.replace([np.inf, -np.inf], 0)
...
def produce_AUC(name, predict_num, choice):
    ...
    creat_numpy_files_predictor_choice(dir_output, name, predict_num, choice)
...
def AUC_wrapper(name):
    ...
    np.save("real_complete_results_final2/AUPRC_"+(name)+".npy",AUPRC)
    np.save("real_complete_results_final2/AUC_"+(name)+".npy",AUC)
    np.save("real_complete_results_final2/PRE_"+(name)+".npy",PRE)
    np.save("real_complete_results_final2/REC_"+(name)+".npy",REC)
    np.save("real_complete_results_final2/FEATIM_"+(name)+".npy",FEATIM)
    np.save("real_complete_results_final2/VAR_"+(name)+".npy",VAR)
